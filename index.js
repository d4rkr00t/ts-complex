let { calculateMaintainabilityFromSource } = require("./lib/main");

console.log(
    calculateMaintainabilityFromSource(
        "temp.ts",
    ...
    import {
      ...
      ACTION_SUBJECT_ID,
      addAnalytics,
      ConversionAttributesAEP,
      EVENT_TYPE,
      INDENT_DIR,
      INDENT_TYPE,
      INPUT_METHOD,
      ListConversionTrackAEP,
      ListConversionUIAEP,
      withAnalytics,
    } from '../../analytics';
    ...
    const getItemAttributes = (state: EditorState, $pos: ResolvedPos) => {
      ...
      const itemIndex: number = $pos.index(
        itemAtPos ? itemAtPos.depth - 1 : undefined,
      );
      return { indentLevel, itemIndex };
    };

    const getConversionAnalyticsAttributes = (
      inputMethod: InputMethod,
      state: EditorState,
    ): ConversionAttributesAEP | undefined => {
      ...
      const parentAtStart = findParentNodeOfTypeClosestToPos($from, [
      ...
      if (!fromAttrs || !toAttrs || !parentAtStart) {
        return;
      }

      const listType = parentAtStart.node.type.name as 'bulletList' | 'orderedList';

      return {
        transformedFrom: listTypeActionSubjectId[listType] as
          | ACTION_SUBJECT_ID.FORMAT_LIST_BULLET
          | ACTION_SUBJECT_ID.FORMAT_LIST_NUMBER,
        fromItemIndex: fromAttrs.itemIndex,
        fromIndentLevel: fromAttrs.indentLevel,
        toItemIndex: toAttrs.itemIndex,
        toIndentLevel: toAttrs.indentLevel,
        inputMethod,
      };
    };
    ...
    export const toggleListCommandWithAnalytics = (
      ...
      return (state, dispatch, view) => {
        if (view && dispatch) {
          const attributes = getConversionAnalyticsAttributes(inputMethod, state);
          if (attributes) {
            const payload: ListConversionTrackAEP | ListConversionUIAEP = {
            ...
            let tr = addAnalytics(state, view.state.tr, {
              ...payload,
